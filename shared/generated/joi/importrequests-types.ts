...
import { importOperationType } from './importoperations-types'
import { categoryImportType } from './categories-types'
import { orderImportType } from './orders-types'
...
const schema = {
  ...
  importRequestStateType: null,
  categoryImportRequestType: null,
  orderImportRequestType: null,
...
schema.importRequestType = Joi.lazy(() => Joi.alternatives([categoryImportRequestType(), orderImportRequestType(), productImportRequestType(), productTypeImportRequestType(), productVariantImportRequestType(), priceImportRequestType(), productVariantPatchRequestType()]))
export function importRequestType() : Joi.AnySchema {
   return schema.importRequestType
}

schema.importRequestStateType = Joi.lazy(() => Joi.object().unknown().keys({
                                   operations: Joi.array().items(importOperationType()).required()
                                }))
export function importRequestStateType() : Joi.AnySchema {
   return schema.importRequestStateType
}
...
schema.orderImportRequestType = Joi.lazy(() => Joi.object().unknown().keys({
                                   resources: Joi.array().items(orderImportType()).required(),
                                   type: importResourceTypeType().only('order').required()
                                }))
export function orderImportRequestType() : Joi.AnySchema {
   return schema.orderImportRequestType
}
